...
static inline void sgemm_micro_v8_m4n24_colmajor_packB(
    ...
    size_t lda, size_t ldb, size_t ldc)
{
    // cover C_M × C_N in 24 x 4 block size
    // j0 means the col of B
    for (size_t j0 = 0; j0 < C_N; j0 += 24) {
        const int jm = (int)std::min<size_t>(24, C_N - j0);     // test if cols is aligned with 24
        const int w0 = std::min(8,  jm);
        const int w1 = std::max(0, std::min(8,  jm - 8));
        const int w2 = std::max(0, std::min(8,  jm - 16));
        const __m256i m0 = (w0==8) ? _mm256_set1_epi32(-1) : mask8(w0);
        const __m256i m1 = (w1==8) ? _mm256_set1_epi32(-1) : mask8(w1);
        const __m256i m2 = (w2==8) ? _mm256_set1_epi32(-1) : mask8(w2);
        ...
        for (size_t i0 = 0; i0 < C_M; i0 += 4) {
            ...
            __m256 c10 = _mm256_setzero_ps(), c11 = _mm256_setzero_ps(), c12 = _mm256_setzero_ps(); // row 1
            __m256 c20 = _mm256_setzero_ps(), c21 = _mm256_setzero_ps(), c22 = _mm256_setzero_ps(); // row 2
            __m256 c30 = _mm256_setzero_ps(), c31 = _mm256_setzero_ps(), c32 = _mm256_setzero_ps(); // row 3

            // stepping by C_K
            for (int k = 0; k < (int)C_K; ++k) {
                const float* bp = Bpack + k*24;  // stride = 24
                // buffer
                __m256 b0 = (w0==8) ? _mm256_loadu_ps(bp +  0) : _mm256_maskload_ps(bp +  0, m0);
            ...
            // write to C
            // compute index
            float* c0 = C + (j0 +  0) * ldc + i0;
            float* c1 = C + (j0 +  8) * ldc + i0;
            float* c2 = C + (j0 + 16) * ldc + i0;

            // lambda function: mask store, in case of border situation
            auto add_row_cols8 = [&](float* colbase, int row, __m256 v, int w_cols) {
                alignas(32) float tmp[8];
                _mm256_store_ps(tmp, v);                 // v[0..7] 是这一行的 8 个列值
                for (int t = 0; t < w_cols; ++t) {
                    // col-major: 同一列相邻两行相差 +1；相邻两列相差 +ldc
                    colbase[t * (int)ldc + row] += tmp[t];
                }
            };

            // row 0..im-1 分别写它自己的三个向量
            if (im >= 1) { add_row_cols8(c0, 0, c00, w0); add_row_cols8(c1, 0, c01, w1); add_row_cols8(c2, 0, c02, w2); }
            if (im >= 2) { add_row_cols8(c0, 1, c10, w0); add_row_cols8(c1, 1, c11, w1); add_row_cols8(c2, 1, c12, w2); }
            if (im >= 3) { add_row_cols8(c0, 2, c20, w0); add_row_cols8(c1, 2, c21, w1); add_row_cols8(c2, 2, c22, w2); }
            if (im >= 4) { add_row_cols8(c0, 3, c30, w0); add_row_cols8(c1, 3, c31, w1); add_row_cols8(c2, 3, c32, w2); }
        }
    }
}
    ...
    size_t lda, size_t ldb, size_t ldc)
{
    ...
    if (C_M >= C_N && C_M >= C_K) {
        size_t M1 = C_M / 2, M2 = C_M - M1;
        sgemm_v8_recursive<BASE>(A,        B,        C,        M1, C_N, C_K, lda, ldb, ldc);
        sgemm_v8_recursive<BASE>(A + M1,   B,        C + M1,   M2, C_N, C_K, lda, ldb, ldc);
    } else if (C_N >= C_M && C_N >= C_K) {
        size_t N1 = C_N / 2, N2 = C_N - N1;
        sgemm_v8_recursive<BASE>(A,        B,              C,              C_M, N1, C_K, lda, ldb, ldc);
        sgemm_v8_recursive<BASE>(A,        B + N1*ldb,     C + N1*ldc,     C_M, N2, C_K, lda, ldb, ldc);
    } else {
        size_t K1 = C_K / 2, K2 = C_K - K1;
        sgemm_v8_recursive<BASE>(A,              B,          C, C_M, C_N, K1, lda, ldb, ldc);
        sgemm_v8_recursive<BASE>(A + K1*lda,     B + K1,     C, C_M, C_N, K2, lda, ldb, ldc);
    }
}

template <const int NUM_THREADS, const int TILE_BASE>
void sgemm_v8(float* a, float* b, float* c,
              const size_t M, const size_t N, const size_t K) {
    // use multi thread to handle this GEMM process
    const int tiles_x = NUM_THREADS / 2;   // 4
    const int tiles_y = 2;                 // 2
...
// ================================================= v9 ====================================================
template<int BASE>
static inline void sgemm_micro_v9_pack_avx_outer_8x6_colmajor(
    const float* __restrict A,  // [M,K], col-major, lda=M
    const float* __restrict B,  // [K,N], col-major, ldb=K
    float* __restrict       C,  // [M,N], col-major, ldc=M
    size_t C_M, size_t C_N, size_t C_K,
...
{
    if (C_M <= (size_t)BASE && C_N <= (size_t)BASE && C_K <= (size_t)BASE) {
        sgemm_micro_v9_pack_avx_outer_8x6_colmajor<BASE>(A, B, C, C_M, C_N, C_K, lda, ldb, ldc);
        return;
    }
    ...
    if (C_M >= C_N && C_M >= C_K) {
        ...
        const size_t M1 = C_M / 2;
        const size_t M2 = C_M - M1;
        sgemm_v9_recursive<BASE>(A,              B,     C,              M1, C_N, C_K, lda, ldb, ldc);
        sgemm_v9_recursive<BASE>(A + M1,        B, C + M1,   M2, C_N, C_K, lda, ldb, ldc);      // now we are at col-major!!!!!!!!!
    } else if (C_N >= C_M && C_N >= C_K) {
        // split N
        const size_t N1 = C_N / 2;
        const size_t N2 = C_N - N1;
        sgemm_v9_recursive<BASE>(A, B,                          C,             C_M, N1, C_K, lda, ldb, ldc);
        sgemm_v9_recursive<BASE>(A, B + N1 * ldb,           C + N1 * ldc,        C_M, N2, C_K, lda, ldb, ldc); // now we are at col-major!!!!!!!!!
    } else {
        // split K: this is outer product!
        const size_t K1 = C_K / 2;
        const size_t K2 = C_K - K1;
        sgemm_v9_recursive<BASE>(A,           B,             C, C_M, C_N, K1, lda, ldb, ldc);
        sgemm_v9_recursive<BASE>(A + K1 * lda,      B + K1,  C, C_M, C_N, K2, lda, ldb, ldc);           // now we are at col-major!!!!!!!!!
    }
}

template <const int NUM_THREADS, const int TILE_BASE>
void sgemm_v9(float* a, float* b, float* c,
              const size_t M, const size_t N, const size_t K) {
    // use multi thread to handle this GEMM process
    const int tiles_x = NUM_THREADS / 2;   // 4
    const int tiles_y = 2;                 // 2
...
py::array_t<float> gemm_f32_v8(
    ...
    {
        py::gil_scoped_release release;
        sgemm_v8<8, 64>(Ap, Bp, Cp, (std::size_t)M, (std::size_t)K, (std::size_t)N);
    }
...
PYBIND11_MODULE(cogemm, m) {
    ...
    m.def("gemm_f32_v8", &gemm_f32_v8, "C = A @ B (float32, col-major, packing B, 4 x 24 tile)");
}